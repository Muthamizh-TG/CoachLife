...
export const PastSessions = () => {
  const { currentUser, sessionHistory, completeSession } = useStore();
  ...
  const draftSessions = mySessions.filter((s) => s.status === 'draft');

  const handleSaveSession = (updatedSession) => {
    // In a real app, this would update the session in the store
              ...
              {pendingSessions.slice().reverse().map((session, index) => (
                ...
                >
                  <div style={{
                    ...
                    <div>
                      <p style={{ fontSize: '12px', color: '#64748B', margin: '0 0 4px 0' }}>Student</p>
                      <p style={{ fontSize: '15px', fontWeight: '600', color: '#111827', margin: '0' }}>{session.student}</p>
                    </div>
                    <div>
