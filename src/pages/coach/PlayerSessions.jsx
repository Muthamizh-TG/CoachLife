...
export const PlayerSessions = () => {
  ...
  const student = players.find(p => p.playerId === playerId);
  
  // Check if this student belongs to the current coach
  if (!student || student.primaryCoach !== currentUser.id) {
    return (
      <Layout>
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            <p className="text-xl font-semibold text-gray-900">Access Denied</p>
            <p className="text-gray-500 mt-2">This student is not assigned to you</p>
            <button 
              onClick={() => navigate(-1)}
  ...
  const studentSessions = getPlayerSessions(playerId);
  
  // Organize sessions by status
  const upcomingSessions = studentSessions.filter(s => s.status === 'draft' || s.status === 'upcoming');
  const draftSessions = studentSessions.filter(s => s.status === 'draft');
  const completedSessions = studentSessions.filter(s => s.status === 'submitted' || s.status === 'completed');
  
  // Calculate session statistics
  const submittedCount = studentSessions.filter(s => s.status === 'submitted').length;
  const totalPoints = studentSessions.reduce((sum, s) => sum + (s.pointsEarned || s.defaultPoints + s.bonusPoints || 0), 0);
  const avgRating = studentSessions.length > 0 
    ? (studentSessions.reduce((sum, s) => sum + (s.rating || 0), 0) / studentSessions.length).toFixed(1)
    : 0;
            ...
            <div style={{
              ...
              {draftSessions.map((session, index) => (
                ...
                >
                  ...
                  <ChevronRight size={20} color="#F6C90E" />
                </Link>
              ))}
            </div>
          </div>
